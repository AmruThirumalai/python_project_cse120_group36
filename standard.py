...
# start row 3
but_squared = tk.Button(calc, text="x\u00b2", command=lambda: add_to_calculation("sqr("), width=5, font=DEFAULT_FONT_STYLE,
                        fg=INK_BLUE, bg=WHITE)
but_squared.grid(row=3, column=0)

but_sqrt = tk.Button(calc, text="\u221ax", command=lambda: add_to_calculation("sqrt("), width=5, font=DEFAULT_FONT_STYLE,
                     fg=INK_BLUE, bg=WHITE)
